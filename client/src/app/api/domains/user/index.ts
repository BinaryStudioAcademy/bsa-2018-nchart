...
export class UserDomainService implements UserDomain {
	...
	register(payload: {
		...
		const s = this.httpService.makeRequest<
			ResponseScheme<UserAuthentication>
		>(new ServiceRequest(RequestType.POST, '/register', null, payload));
	...
	login(payload: Login): Observable<ResponseScheme<UserAuthentication>> {
		const s = this.httpService.makeRequest<
			ResponseScheme<UserAuthentication>
		>(new ServiceRequest(RequestType.POST, '/login', null, payload));

		return s;
	}
	update(payload: User): Observable<ResponseScheme<User>> {
		const s = this.httpService.makeRequest<ResponseScheme<User>>(
			new ServiceRequest(RequestType.PUT, `/${payload.id}`, null, payload)
		);
